package ru.sinergia.lessons.homework6;
 ...
 */
    ...
    public static abstract class Instrument implements Playable{
        ...
        public List<String> playWith(Playable playable){
            String res = this.getClass().getSimpleName().toLowerCase()
             + ".playWith("
    ...
    public static class Orchestra implements Playable{
        ...
        public Orchestra(Instrument... inst) {
            for(int i = 0; i < inst.length; i++) {
                System.out.println(" arg " + i + ": " + inst[i]);
                instruments.set(i, inst[i]);
            }
